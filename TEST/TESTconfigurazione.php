...
class TESTconfigurazione{
    ...
    public function test_modifica_formula(){
        ...
        try {
            ...
            // ASSERT: L'oggetto è stato creato
            $this->assertTrue(
                $config !== null,
                "Oggetto ModificaParametriConfigurazione creato"
            );
            
            // NOTA: Non testiamo la scrittura su file perché non vogliamo
            // modificare i file di produzione durante i test!
            
        } finally {
            // RIPRISTINA il file originale (per sicurezza)
            file_put_contents($path, $backup);
            echo "  <span style='color: blue;'>ℹ File originale ripristinato (test sicuro)</span><br>";
        }
    }
